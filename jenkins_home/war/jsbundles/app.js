...
function generateDropdowns() {
  behavior_shim.specify(".hoverable-model-link", "-hoverable-dropdown-", 1000, element => utils.generateDropdown(element, instance => {
    const href = element.href;
    if (element.items) {
      instance.setContent(utils.generateDropdownItems(element.items));
      return;
    }
    fetch(path.combinePath(href, "contextMenu")).then(response => response.json()).then(json => instance.setContent(utils.generateDropdownItems(mapChildrenItemsToDropdownItems(json.items)))).catch(error => console.log(`Jumplist request failed: ${error}`)).finally(() => instance.loaded = true);
  }, false, {
    trigger: "mouseenter",
    offset: [-16, 10],
    animation: "tooltip",
